...
class ChatbotSeed {
  ChatbotSeed({AnalyticsSeed? analytics, WorkoutSeed? workout})
      : _analytics = analytics ?? AnalyticsSeed(),
        _workout = workout ?? WorkoutSeed();

  final AnalyticsSeed _analytics;
  ...
  String buildProgressAuditReply() {
    final formChange =
        _analytics.postureTrend.last - _analytics.postureTrend.first;
    final volumeChangePct =
        ((_analytics.volumeTrend.last - _analytics.volumeTrend.first) /
                _analytics.volumeTrend.first *
                100)
            .round();
    final weakest =
        _workout.history.reduce((a, b) => a.accuracy < b.accuracy ? a : b);
    return 'Over the last ${_analytics.postureTrend.length} sessions your average form score '
