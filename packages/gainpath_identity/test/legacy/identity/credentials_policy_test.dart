...
void main() {
  ...
  test('valid sign-in credentials produce no errors', () {
    expect(
        policy.validate(email: 'a@b.co', password: 'demo1234').isValid, isTrue);
  });
  ...
  test('malformed email is rejected', () {
    expect(policy.validate(email: 'not-an-email', password: 'x').email,
        'Enter a valid email address.');
  });

  test(
      'registration needs a name, an 8-char password and a matching confirmation',
      () {
    final e = policy.validate(
        email: 'a@b.co',
        password: 'short',
        confirm: 'other',
        registering: true);
